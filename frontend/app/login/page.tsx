...
import { apiLogin, apiSocialRedirect, apiGetAuthSettings } from '@/lib/api';
...
export default function LoginPage() {
  ...
  useEffect(() => {
    apiGetAuthSettings().then((res) => {
      setSocialToggles({
        google: !!res.settings.google_active,
        facebook: !!res.settings.facebook_active,
        apple: !!res.settings.apple_active,
      });
    }).catch(console.error);
  }, []);
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white flex" dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 w-full">
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="max-w-md">
            <h2 className="text-4xl font-bold mb-4 leading-tight">
              {isRTL ? 'أنشئ قصصاً سينمائية' : 'Create cinematic stories'}
              <br />
              <span className="bg-gradient-to-r from-yellow-400 to-pink-500 bg-clip-text text-transparent">
                {isRTL ? 'لأطفالك' : 'for your children'}
              </span>
            </h2>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6">
        ...
        >
          ...
          <h1 className="text-3xl font-bold mb-2">{isRTL ? 'تسجيل الدخول' : 'Sign In'}</h1>
          <p className="text-gray-400 mb-8">{isRTL ? 'أهلاً بعودتك! سجل دخولك للمتابعة.' : 'Welcome back! Sign in to continue.'}</p>

          {/* Social Auth */}
          {(socialToggles.google || socialToggles.facebook || socialToggles.apple) && (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
                {(['google', 'facebook', 'apple'] as const).map((provider) => {
                  if (!socialToggles[provider]) return null;
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5">
                <label className="text-sm font-medium text-gray-300">{isRTL ? 'كلمة المرور' : 'Password'}</label>
                <Link href="/forgot-password" className="text-xs text-yellow-500 hover:text-yellow-400 transition-colors">{isRTL ? 'نسيت كلمة المرور؟' : 'Forgot password?'}</Link>
              </div>
            ...
            >
              {loading ? <Loader2 size={20} className="animate-spin" /> : isRTL ? 'تسجيل الدخول' : 'Sign In'}
            </button>
          </form>

          <p className="mt-6 text-center text-gray-400 text-sm">
            {isRTL ? 'ليس لديك حساب؟ ' : "Don't have an account? "}
            <Link href="/register" className="text-yellow-400 hover:text-yellow-300 font-medium transition-colors">{isRTL ? 'سجل الآن' : 'Sign up'}</Link>
          </p>
        </motion.div>
      </div>
    </div>
