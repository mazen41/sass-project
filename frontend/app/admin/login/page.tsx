...
import { apiLogin, apiRegister, apiSocialRedirect, apiGetAuthSettings } from '@/lib/api';
...
export default function AdminLoginPage() {
  ...
  useEffect(() => {
    apiGetAuthSettings().then((res) => {
      setAllowSocial(res.settings.allow_admin_social_auth);
      setAllowSignup(res.settings.allow_admin_signup);
      setSocialToggles({
        google: !!res.settings.google_active,
        facebook: !!res.settings.facebook_active,
        apple: !!res.settings.apple_active,
      });
      if (!res.settings.allow_admin_signup && mode === 'register') {
        setMode('login');
      }
    }).catch(console.error);
  }, [mode]);
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white flex items-center justify-center p-4" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      >
        ...
        {/* Social Auth */}
        {allowSocial && (socialToggles.google || socialToggles.facebook || socialToggles.apple) && (
          <>
